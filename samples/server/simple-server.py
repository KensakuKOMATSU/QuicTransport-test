...
Example use:
  python3 simple-server.py certificate.pem certificate.key
...
class CounterHandler:
    def __init__(self, proto, connection) -> None:
        self.proto = proto
...
class QuicTransportProtocol(QuicConnectionProtocol):
    ...
    def process_client_indication(self) -> None:
        ...
        if path.path == '/counter':
            self.handler = CounterHandler( self, self._quic)
        else:
